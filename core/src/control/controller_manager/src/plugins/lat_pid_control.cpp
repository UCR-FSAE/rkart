...
namespace roar
{
    namespace control
    {
        ...
        public:
            void initialize(nav2_util::LifecycleNode *node) override
            {
                ...
                config_ = LatConfig{
                    ...
                    this->node().declare_parameter<bool>("lat_control.debug", false),
                };
                ...
                RCLCPP_INFO_STREAM(logger_, "[LatPIDControllerPlugin]: "
                                                ...
                                                << "\nmin_dist: " << config_.min_dist_
                                                << "\ndebug: " << config_.debug_ ? "true" : "false"
                                                );
            }

            bool configure(const ControllerManagerConfig::SharedPtr config) override
            {
                RCLCPP_INFO_STREAM(logger_, "configuring");
            ...
            {   
                // RCLCPP_DEBUG_STREAM(logger_, "updating");
                path_ = std::make_shared<nav_msgs::msg::Path>(state->path_ego_centric);
                // RCLCPP_DEBUG_STREAM(logger_, "path size: " << path_->poses.size());
                if (state->vehicle_state == nullptr)
                {
                    RCLCPP_ERROR_STREAM(logger_, "vehicle state is null");
                    return false;
                }
                //problem here
                lat_state().current_speed = state->vehicle_state->vehicle_status.speed;
                // RCLCPP_DEBUG_STREAM(logger_, "current speed: " << lat_state().current_speed);
                return true;
            }
            ...
            {   
                if (path_ == nullptr)
                {
                    RCLCPP_ERROR_STREAM(logger_, "path is null");
                    return false;
                }

                if (lat_state().last_pid_time.seconds() == 0.0)
                {
                    RCLCPP_WARN_STREAM(logger_, "first iteration, dt uncomputable, not updating Lat PID");
                ...
                RCLCPP_DEBUG_STREAM(logger_,
                                    "[LatPIDControllerPlugin]: path[next_waypoint] x= " << path_->poses[next_waypoint].pose.position.x
                ...
                if (this_pid_time <= lat_state().last_pid_time)
                {
                    lat_state().last_pid_time = this_pid_time;
                    // RCLCPP_WARN_STREAM(logger_, "this_pid_time [" << this_pid_time.seconds() << "] is less than or equal to last_pid_time [" << lat_state().last_pid_time.seconds() << "], skipping this iteration, try increasing dt");
                    return true;
                }
                ...
                RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin]: steering_error: " << steering_error << " dt_sec: " << dt_sec << " steering_output: " << steering_output);
            ...
            void p_updatePID()
            {
                ...
                RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin] kp: " << k_p_value << " ki: " << k_i_value << " kd: " << k_d_value);
        ...
        protected:
            ...
            {
                int next_waypoint = 0;
                // RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin]: path size: " << path.poses.size());
                for (int i = 0; i < path.poses.size(); i++)
                {
                    float dist = sqrt(pow(path.poses[i].pose.position.x, 2) + pow(path.poses[i].pose.position.y, 2));
                    RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin]: i:" << i << " dist: " << dist << " min_dist: " << config_.min_dist_ << " x: " << path.poses[i].pose.position.x << " y: " << path.poses[i].pose.position.y);
                    next_waypoint = i;
                ...
                RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin]: next_waypoint index: " << next_waypoint << " dist: " << dist << " min_dist: " << config_.min_dist_);
            ...
            double p_calcAngularError(nav_msgs::msg::Path path, int next_waypoint)
            {
                ...
                if (next_waypoint <= path.poses.size() - 1)
                {
                    ...
                    RCLCPP_DEBUG_STREAM(logger_, "[LatPIDControllerPlugin] angular_error: " << angular_error << " next_waypoint.x: " << path.poses[next_waypoint].pose.position.x << " next_waypoint.y: " << path.poses[next_waypoint].pose.position.y);
                }
