        ...
        namespace local
        {
            ...
            protected:
                ...
                void publishBestPathVisualizationMarker(const nav_msgs::msg::Path::SharedPtr best_path)
                {
                    ...
                    int interval = best_path->poses.size() / 10;
                    for (int i = 0; i < best_path->poses.size(); i+= interval)
                    {
                        ...
                        marker.header.frame_id = best_path->header.frame_id;
                        ...
                        RCLCPP_DEBUG_STREAM(get_logger(), "Publishing best path marker at " << point.x << ", " << point.y);
                    }
