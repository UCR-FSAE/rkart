...
#include "global_planning/planners/hybrid_a_star/display_tools.h"
#include "global_planning/planners/hybrid_a_star/timer.h"
#include "global_planning/planners/hybrid_a_star/trajectory_optimizer.h"
#include <sstream>
#include <iostream>
...
void HybridAStar::Init(double x_lower, double x_upper, double y_lower, double y_upper,
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] STATE_GRID_SIZE_X_: " << STATE_GRID_SIZE_X_ << " STATE_GRID_SIZE_Y_: " << STATE_GRID_SIZE_Y_);
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] MAP_GRID_SIZE_X_: " << MAP_GRID_SIZE_X_ << " MAP_GRID_SIZE_Y_: " << MAP_GRID_SIZE_Y_);
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] map_data_ init");
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] state_node_map_ init");
}

Vec3d HybridAStar::CapCoordinatesToMapSizeIfNeeded(const Vec3d &pt)
{
    Vec3d pt_capped = pt;
    if (pt_capped.x() < map_x_lower_)
    {
        pt_capped.x() = map_x_lower_;
    }
    else if (pt_capped.x() > map_x_upper_)
    {
        pt_capped.x() = map_x_upper_;
    }

    if (pt_capped.y() < map_y_lower_)
    {
        pt_capped.y() = map_y_lower_;
    }
    else if (pt_capped.y() > map_y_upper_)
    {
        pt_capped.y() = map_y_upper_;
    }
...
inline bool HybridAStar::LineCheck(double x0, double y0, double x1, double y1) {
    ...
    for (unsigned int i = 0; i < N; ++i) {
        // do not check boundary, because the boundary is not obstacle
        if (steep)
        {
...
// return false if there is obstacle
...
bool HybridAStar::SetObstacle(unsigned int x, unsigned int y) {
...
void HybridAStar::GetNeighborNodes(const StateNode::Ptr &curr_node_ptr,
    ...
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] Current Node: " << curr_node_ptr->state_.x() << ", " << curr_node_ptr->state_.y() << ", " << curr_node_ptr->state_.z());
    for (int i = -steering_discrete_num_; i <= steering_discrete_num_; ++i) {
        ...
        for (int j = 1; j <= segment_length_discrete_num_; j++) {
            ...
            if (!CheckCollision(x, y, theta)) {
                RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] Collision detected at " << x << ", " << y << ", " << theta);
                has_obstacle = true;
                break;
            }
        }
        // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] move_step_size_: " << move_step_size_ << " | phi: " << phi << " | x: " << x << " | y: " << y << " | theta: " << theta << " | has_obstacle: " << has_obstacle << " | segment_length_discrete_num_: " << segment_length_discrete_num_);
        // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] intermediate_state.size(): " << intermediate_state.size() << " | has_obstacle: " << has_obstacle << " | segment_length_discrete_num_: " << segment_length_discrete_num_ );
...
bool HybridAStar::Search(const Vec3d &start_state, const Vec3d &goal_state, int max_iterations) {
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] Start Search");
    ...
    RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] goal_node_ptr configured");
    ...
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] start_node_ptr configured");

    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] state_node_map_ x:" << STATE_GRID_SIZE_X_ << " y:" << STATE_GRID_SIZE_Y_ << " z:" << STATE_GRID_SIZE_PHI_);
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] start_grid_index x:" << start_grid_index.x() << " y:" << start_grid_index.y() << " z:" << start_grid_index.z());
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] goal_grid_index x:" << goal_grid_index.x() << " y:" << goal_grid_index.y() << " z:" << goal_grid_index.z());
    ...
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "[HybridAStar] state_node_map_ configured");
    ...
    // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "Start iterating");
    int count = 0;
    while (!openset_.empty()) {
        ...
        // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "Dist To Goal: " << dist_to_goal << " | shot_distance_: " << shot_distance_);

        if (dist_to_goal <= shot_distance_)
        {
            // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "In shot distance");
            ...
            if (AnalyticExpansions(current_node_ptr, goal_node_ptr, rs_length)) {
                ...
                stringStream << "ComputeH use time(ms): " << compute_h_time << std::endl;
                stringStream << "check collision use time(ms): " << check_collision_use_time << std::endl;
                stringStream << "GetNeighborNodes use time(ms): " << neighbor_time << std::endl;
                stringStream << "average time of check collision(ms): "
                             << (num_check_collision != 0 ? check_collision_use_time / num_check_collision : 0)
                             << std::endl;
                RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), stringStream.str());

                RCLCPP_INFO_STREAM(
                    rclcpp::get_logger("NavPlanner"),
                    "\033[1;32m --> Time in Hybrid A star is " << search_used_time.End() << " ms, path length: " << path_length_ << "  \033[0m");
        ...
        // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "Not in shot distance");
        ...
        // RCLCPP_DEBUG_STREAM(rclcpp::get_logger("NavPlanner"), "GetNeighborNodes use time(ms): " << neighbor_time << " | found " << neighbor_nodes_ptr.size() << " neighbors");
        ...
        if (count > max_iterations)
        {
            RCLCPP_WARN_STREAM(
                rclcpp::get_logger("NavPlanner"),
                "\033[1;32m --> Max Steps [" << max_iterations << "] reached Time in Hybrid A star is " << search_used_time.End() << " ms, path length: " << path_length_ << "  \033[0m");
            return false;
        }
    }
